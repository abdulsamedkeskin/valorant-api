...
def shop():
  ...
  for i in r['FeaturedBundle']['Bundles']:
    ...
    _['items'] = data__
    data_.append(_)
  data.append({"type": "bundle", "remaining_time_in_seconds": r['FeaturedBundle']['BundleRemainingDurationInSeconds'], "data": data_})
  data_ = []
  for i in r['SkinsPanelLayout']['SingleItemOffers']:
    _ = next(item for item in contents[0]['skinLevels'] if item["uuid"] == i)
    data_.append(_)     
  data.append({"type": "single","remaining_time_in_seconds": r['SkinsPanelLayout']['SingleItemOffersRemainingDurationInSeconds'], "data": data_ }) 
  return data
